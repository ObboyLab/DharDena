CREATE TABLE `Dhar`.`repayments` (
  ...
  `from_active_load_id` INT NOT NULL,
  ...
  INDEX `from_active_load_id_idx` (`from_active_load_id` ASC) VISIBLE,
  INDEX `to_investor_active_investment_id_idx` (`to_investor_active_investment_id` ASC) VISIBLE,
  CONSTRAINT `from_active_load_id`
    FOREIGN KEY (`from_active_load_id`)
